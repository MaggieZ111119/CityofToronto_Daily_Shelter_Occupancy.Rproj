...
## Data Breakdown
As shown in @shelter-full-table, The four cities being observed in the dataset are Toronto, Etobicoke, North York, and Scarborough. The sectors recorded include Co-ed (mixture), Families, Men, Women, and Youth. 
```{r}
#| label: shelter-full-table
...
The following table provides a summary of the average, maximum, and minimum occupancy, as well as capacity metrics for each city.
```{r}
#| label: shelter-by-city
...
kable(combined_summary, 
             col.names = c("City", 
                           ...
                           "Min Capacity"))
```
The relastion ship of two key feature, occupancy and capacity, are essential for understaidng current shelter servise system. The relashipship is reflected in @fig-occupancy-capacity-relashionship. More learning on the shelter servise and thier trend, and how they reflect the fomeless population are done, and will be discuss in the next section @sec-result.
```{r}
